...
import { groupMyMemberId, plannedAccessFilter, shareGroupAccessFilter } from "@/lib/share-access";
...
export default async function PlanificadosPage() {
  ...
  const [rows, accounts, categories, tags, gruposRaw, personas, { lista: quotes }] = await Promise.all([
    // Visible si es propio, o si está compartido con un grupo al que esta cuenta tiene acceso
    // (dueña o colaboradora vinculada) -- edición conjunta de planificados compartidos.
    prisma.planned.findMany({
      where: { done: false, ...plannedAccessFilter(userId) },
      orderBy: { dueDate: "asc" },
      include: { category: true, tags: true, shareGroup: { include: { members: true, collaborators: true } } },
    }),
    ...
    prisma.shareGroup.findMany({
      where: { ...shareGroupAccessFilter(userId), archived: false },
  ...
  // El picker de grupos (para tildar "¿pertenece a un presupuesto compartido?" al crear/editar) necesita
  // saber, por cada grupo, cuál integrante es "yo" desde la perspectiva de ESTA cuenta -- el dueño
  // original o, si es colaboradora vinculada, el integrante al que quedó vinculada.
  const groups = gruposRaw.map((g) => ({ ...g, myMemberId: groupMyMemberId(g, userId) }));
  ...
  const miParte = (i: (typeof items)[number]) => {
    ...
    const miMemberId = groupMyMemberId(i.shareGroup, userId);
    const miPorcentaje = i.shareGroup.members.find((m) => m.id === miMemberId)?.defaultPercent;
